...
/// [CustomReorderableSliverList], replace [SliverChildBuilderDelegate] with this
/// class.
///
/// See also:
///
...
/// [CustomReorderableSliverList], replace [SliverChildListDelegate] with this class.
///
/// See also:
///
...
class CustomReorderableSliverList extends StatefulWidget {
  ...
  final bool enabled;
  final GlobalKey<DraggableFloatingActionButtonState> fabKey;

  @override
  _CustomReorderableSliverListState createState() =>
      _CustomReorderableSliverListState();
}

class _CustomReorderableSliverListState
    extends State<CustomReorderableSliverList>
    with
        TickerProviderStateMixin<CustomReorderableSliverList>,
        ReorderableMixin {
...
//    final GlobalObjectKey keyIndexGlobalKey = GlobalObjectKey(toWrap.key);
    ...
    void initializeOnDragStarted(
        Widget _draggingWidget, int _index, Size _draggingFeedbackSize) {
      this._draggingWidget = _draggingWidget;
      this._dragStartIndex = _index;
      this._ghostIndex = _index;
      this._currentIndex = _index;
      this._entranceController.value = 1.0;
      this._draggingFeedbackSize = _draggingFeedbackSize;
    }

    // Starts dragging toWrap.
    void onDragStarted() {
      setState(() {
        initializeOnDragStarted(toWrap, index, draggableContext.size);
      });
    }

    // Places the value from startIndex one space before the element at endIndex.
    void _reorder(int startIndex, int endIndex) {
      if (_isDraggingActionButton) return;
...
//              child: _makeAppearingWidget(toWrap)
    ...
    return Builder(builder: (BuildContext context) {
        ...
        onWillAccept: (int toAccept) {
          bool willAccept =
              (_dragStartIndex == toAccept && toAccept != index) ||
                  (_isDraggingActionButton && toAccept == -2);
          setState(() {
            if (toAccept == -2 && !_isDraggingActionButton) {
              _isDraggingActionButton = true;
              final size = Size(context.size.width, 72);
              initializeOnDragStarted(widget.fabKey.currentWidget, index, size);
              widget.fabKey.currentState.onDragEnded = () {
                var temp = _currentIndex;
                onDragEnded();
                _isDraggingActionButton = false;
                return temp;
              };
            }
            if (willAccept) {
      ...
      Widget spacing = (_isDraggingActionButton)
          ? DottedBorder(
...
class DraggableFloatingActionButtonState
  ...
  int Function() _onDragEnded = () {
    return -1;
  };
  bool getCancelled = false;

  @override
  Widget build(BuildContext context) {
    var child = widget.floatingActionButton;
    ...
    return Draggable<int>(
      ...
      data: -2,
      childWhenDragging: _CancelDragging(
        getCancelled: getCancelled,
        setState: (state) {
          setState(() => getCancelled = state);
        },
      ),
      onDragStarted: () => setState(() => getCancelled = false),
      onDragCompleted: () {
        if (getCancelled)
          _onDragEnded();
        else
          _callAddTaskPanel(_onDragEnded)();
      },
  ...
  set onDragEnded(int Function() onDragEnded) {
    setState(() {
      this._onDragEnded = onDragEnded;
    });
  }

  Function _callAddTaskPanel(Function onDragEnded) {
    return () {
      int posBefore = onDragEnded();
      showAddTaskPanel(context, posBefore: posBefore);
    };
  }
}

class _CancelDragging extends StatelessWidget {
  const _CancelDragging({
    this.getCancelled,
    this.setState,
    Key key,
  }) : super(key: key);

  final void Function(bool state) setState;
  final bool getCancelled;
  @override
  Widget build(BuildContext context) {
        ...
        return Container(
          ...
          child: Center(
            child: AnimatedContainer(
              ...
              width: getCancelled ? 52 : 40,
              height: getCancelled ? 52 : 40,
              decoration: BoxDecoration(
