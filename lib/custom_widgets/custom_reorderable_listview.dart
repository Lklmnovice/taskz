...
class _ReorderableListContentState extends State<_ReorderableListContent> with TickerProviderStateMixin<_ReorderableListContent> {
  ...
  // custom modification
  // Whether or not we are currently dragging action button
  ...
  Widget _wrap(Widget toWrap, int index, BoxConstraints constraints) {
    ...
    void onDragStarted() {
      setState(() {
        initializeOnDragStarted(toWrap.key, index, keyIndexGlobalKey.currentContext.size);
      });
    }
    ...
    return Builder(builder: (BuildContext context) {
      return DragTarget<Key>(
        ...
        onWillAccept: (Key toAccept) {
          ...
          setState(() {
            //custom modification
            if (toAccept is GlobalKey<DraggableFloatingActionButtonState> && _isDraggingActionButton == false) {
              _isDraggingActionButton = true;
              initializeOnDragStarted(toAccept, index, Size(30, 30));
              toAccept.currentState.onDragEnded = () {
                setState(() {
                  _isDraggingActionButton = false;
                  _ghostController.reverse(from: 0.1);
                  _entranceController.reverse(from: 1);
                  _dragging = null;
                });
...
class DraggableFloatingActionButton extends StatefulWidget {
  ...
  @override
  DraggableFloatingActionButtonState createState() => DraggableFloatingActionButtonState();
}

class DraggableFloatingActionButtonState extends State<DraggableFloatingActionButton> {
  int Function() _onDragEnded = () {return -1;};
  ...
  Widget build(BuildContext context) {
    ...
    return Draggable<Key>(
      ...
      onDragCompleted: _callAddTaskPanel(_onDragEnded),
  ...
  Function _callAddTaskPanel(Function onDragEnded) {
    return () {
      ...
      Navigator.of(context).pushNamed('/add_task', arguments: posBefore);
    };
  }
}


class _CancelDragging extends StatefulWidget {
  const _CancelDragging({
    Key key,
  }) : super(key: key);

  @override
  __CancelDraggingState createState() => __CancelDraggingState();
}

class __CancelDraggingState extends State<_CancelDragging> {
  ...
  Widget build(BuildContext context) {
    return DragTarget<Key>(
      ...
      return Container(
        ...
        child: Center(
          child: AnimatedContainer(
            ...
            width: _isActive ? 52 : 40,
            height: _isActive ? 52 : 40,
      ...
      onWillAccept: (data) { _isActive = true; return false;},
      onLeave: (data) => _isActive = false,
//      onAccept: (data) => print('dragging canceled '),
    );
  }
}